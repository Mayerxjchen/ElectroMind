...
设计：
- Runner 只关心 Messages，本身不认 conversation_id。
- ConversationStore 负责 <id> ↔ Messages 的读写；把它注入 Runner，
  Runner 在 arun/session 里根据 conversation_id 自动 load/save。
- 默认后端是 JSONL，落到项目根下的 .pagent/conversations/<id>.jsonl；
  换 SQLite 只需把 store 换掉，Runner 无感。
- 保存粒度：每个 TurnEnd flush 一次。中间态多写一点，换崩溃安全。
"""
...
class JsonlConversationStore:
    """一 conversation 一个 jsonl 文件，易 diff 易查看。"""
