...
def trace_title(source: str) -> str:
    ...
    try:
        ...
        return f"electromind trace · {source}"
...
def write_trace(
    ...
    *,
    ...
    if output is None:
        output = Path("electromind-trace.html")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "source",
        help="messages.jsonl path, thread id under .electromind/threads/, or - for stdin JSONL",
    )
    parser.add_argument(
        "-f",
        "--format",
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="write to file instead of stdout (html default: electromind-trace.html)",
    )
    parser.add_argument(
        "--open",
        action="store_true",
