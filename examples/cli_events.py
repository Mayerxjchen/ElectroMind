"""Interactive CLI with full ``arun_events()`` stream (tools, reasoning).

Usage:
    export DEEPSEEK_API_KEY="your-key"
    uv run examples/cli_events.py
"""
...
from pagent import ReasoningDelta, RunEnd, TextDelta, ToolCallBegin, ToolResult
...
async def stream_reply(
    ...
    *,
    ...
    # Which block is currently being printed (controls line breaks between sections).
    mode = None
    had_text = False
    had_tools = False
    ...
    async for event in agent.arun_events(user_input, **run_kwargs):
