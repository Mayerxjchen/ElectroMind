...
Wire 是对 `arun_events()` 的薄序列化层，见 [wire.zh-CN.md](./wire.zh-CN.md)。在 Python 里也可对单个事件 `encode_event_line(event)` 再发到 socket。
...
- **Wire 协议** — JSON-RPC 2.0 通知 + NDJSON：[wire.zh-CN.md](./wire.zh-CN.md)。可用 `Agent.arun_wire()` 或 `encode_event_line()`。
