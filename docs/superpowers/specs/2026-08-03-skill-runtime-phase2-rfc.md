...
IGNORED_DIRS             = .git .hg .svn .venv __pycache__ node_modules dist build 及隐藏目录
ADOPTED_ROOT_ENTRIES     = root 直接子级目录符号链接，仅当其真实目标位于当前作用域允许的
                            trusted path 集合（项目根 / 用户主目录 / 显式 external_skill_roots）内
                            时采纳为子根；嵌套符号链接一律跳过；可独立关闭（默认无 external roots 即关闭）
```
...
When the same physical Skill is discovered through both project and builtin
roots, physical deduplication selects the **project provenance** as the
effective scope.

Trust is evaluated from the **effective scope**.  Builtin provenance does
NOT grant builtin trust to a mutable project Skill.

Therefore, repository-hosted Skills require workspace trust even when the
same files are also visible through a development-mode builtin root.

> 核心原则：**去重可以合并发现来源（discovery provenance），但不能合并或提升信任（trust）。**

安全模型：

```python
effective_scope = highest_precedence_scope          # project > user > admin > builtin
effective_trust = trust_policy(effective_scope, workspace_state)   # 不取 max(source.trust)
```

职责分离：

```text
Discovery  → 确认物理身份与全部 provenance
Resolution → 决定 effective scope
Trust      → 根据 effective scope 与 workspace trust 判断可否激活
```

绝不采用"最高信任来源"模型（`effective_trust = max(source.trust)`）——那会让一个
不可信的 Project Skill 因碰巧与 builtin 路径重叠而获得信任，构成信任绕过。
