...
**首次打开**若尚未安装 pagent 或未配置 API Key，会强制弹出 **首次设置**（三步：环境 → API Key → 沙箱），完成前无法使用主界面。配置齐全后可在用户菜单再次打开向导。设置页里的 **环境自检** 是另一块：用来诊断状态与磁盘占用，不是向导。

引导会检测：

- `uv` / `pagent` 是否在 PATH（可一键 `uv tool install pagent` 或复制命令）
- 是否已配置 API Key（可写入 `~/.pagent/pagent.toml`）
- 可选：`docker` / `podman` 与 `pagent:latest` 镜像（仅 container 沙箱需要）

点 **稍后配置** 可跳过；之后发消息前仍需自行配置 Key。
...
若未配置，发送消息后会出现错误提示。标题栏 **设置**（齿轮）可进行 **环境自检**（状态灯 + `~/.pagent` / 镜像磁盘占用）并查看 `pagent.toml` 概览；配置内容需在文本编辑器中修改。
...
| 入口 | 作用 |
| --- | --- |
| **齿轮** | 环境自检（状态灯 + 磁盘占用）+ 查看 `pagent.toml` |
| **书本图标** | 在浏览器打开本文档站 |
| **用户菜单 → 扫码看文档** | 手机扫码阅读文档 |
