...
给要改库内部的贡献者看的。用库的先去 [文档首页](/sc/) 或 [赶紧上手](./guide/quick-start)，莫一上来就翻源码，打脑壳。
...
- 并行工具、RAG、MCP、内置文件/Shell、多模态、检查点 — 业务层自己整，库莫胀。
- `run()` 非流式；流式用 `arun()` / `arun_events()`。

## 本地开发

先把环境 **理抹** 整齐，再 **架势**（[`uv` 是啥子？看这儿](https://docs.astral.sh/uv/)）：
...
`pytest` 全绿、pre-commit **过一道** 没得问题，再提 PR — 这样办 **落教**、**稳当**。还差一脚的，**攒把劲** 哈。
...
推 `main` 后 [docs.yml](https://github.com/SyncLionPaw/pagent/blob/main/.github/workflows/docs.yml) 把 `docs/.vitepress/dist/` 发到 **`gh-pages`**。仓库 **Settings → Pages** 选 **gh-pages / root**，你 **经佑** 到起发布配置就安逸。

## 更多

- [事件类型](./events)
- [reasoning_content 例子](./reasoning)
- [Hooks 计划](../plans/hooks.md)（还没实现，先摆在这儿）
