...
# ── backend_type_name 纯函数测试（无需异步 / 沙箱） ──
...
FakeLocal.__name__ = "LocalBackend"


class FakeDocker: ...


FakeDocker.__name__ = "DockerBackend"


class FakePodman: ...


FakePodman.__name__ = "PodmanBackend"


class FakeSsh: ...


FakeSsh.__name__ = "SshBackend"


class FakeUnknown: ...


FakeUnknown.__name__ = "UnknownThing"


def test_backend_type_local():
    assert backend_type_name(FakeLocal()) == "local"


def test_backend_type_docker():
    assert backend_type_name(FakeDocker()) == "docker"


def test_backend_type_podman():
    assert backend_type_name(FakePodman()) == "podman"


def test_backend_type_ssh():
    assert backend_type_name(FakeSsh()) == "ssh"


def test_backend_type_unknown_falls_back_to_lowercase_classname():
    assert backend_type_name(FakeUnknown()) == "unknownthing"


def test_backend_type_unwraps_guard():
    """BackendGuard 应被正确解包，返回内部 backend 的类型名。"""
    guard = BackendGuard(FakeLocal())
    assert backend_type_name(guard) == "local"


def test_backend_type_one_level_unwrap():
    """backend_type_name 只解包一层 BackendGuard（生产环境 Guard 不会嵌套自己）。"""
    guard = BackendGuard(FakeDocker())
    # guard.inner → FakeDocker() → class_name "DockerBackend" → "docker"
    assert backend_type_name(guard) == "docker"


def test_backend_type_empty_string_on_none():
    """防御性：没有 inner 的对象应平稳降级。"""

    class BareBackend:
        pass

    BareBackend.__name__ = "BareBackend"
    # 没有 inner，getattr 返回自己
    assert backend_type_name(BareBackend()) == "barebackend"
