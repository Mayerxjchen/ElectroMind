"""electromind home 两模式：生产 ~/.electromind / 开发 <root>/.electromind。"""
...
def test_dev_mode_ignores_root_legacy_toml(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    root = tmp_path / "proj"
    root.mkdir()
    (root / "electromind.toml").write_text(
        "[provider]\nmodel = 'x'\n", encoding="utf-8"
    )
    activate_home("dev", root)
    # 只认 <root>/.electromind/config.toml；根目录遗留的 electromind.toml 不再被采用。
    assert find_home_config() is None
    (root / ".electromind").mkdir()
    (root / ".electromind" / "config.toml").write_text("", encoding="utf-8")
    assert find_home_config() == root / ".electromind" / "config.toml"
