...
默认 skip；设置 ``DEEPSEEK_API_KEY`` 后才会跑::
    ...
    DEEPSEEK_API_KEY=sk-... uv run pytest tests/test_electromind_e2e_live.py -q

可选环境变量：

- ``ELECTROMIND_E2E_MODEL``：模型名（默认 ``deepseek-v4-flash``）。
- ``ELECTROMIND_E2E_TIMEOUT``：单轮超时秒数（默认 180）。
"""
...
async def test_live_agent_runs_full_loop_with_sandbox_tools(tmp_path, monkeypatch):
    ...
    model = os.getenv("ELECTROMIND_E2E_MODEL", "deepseek-v4-flash")
    timeout = float(os.getenv("ELECTROMIND_E2E_TIMEOUT", "180"))
    ...
    try:
        ...
        async with asyncio.timeout(timeout):
            async for event in runner.run(
                "请在工作目录创建 hello.txt，内容写 electromind-e2e，然后读出来确认。"
            ):
                events.append(event)
    finally:
        await runner.close()
    ...
    # 5) 对话落盘
    assert (
        tmp_path / ".electromind" / "threads" / "e2e-live" / "messages.jsonl"
    ).is_file()
