...
async def test_local_files_list(tmp_path):
    async with await Sandbox.create(backend="local", workdir=str(tmp_path)) as box:
        ...
        # Filter out hidden files (e.g. .electromind_audit.jsonl)
...
async def test_local_files_list_skips_broken_symlink(tmp_path):
    async with await Sandbox.create(backend="local", workdir=str(tmp_path)) as box:
        ...
        assert [e.name for e in entries if not e.name.startswith(".")] == ["a.txt"]
...
async def test_str_replace_tool_wiring(tmp_path):
    async with await Sandbox.create(backend="local", workdir=str(tmp_path)) as box:
        ...
        assert (tmp_path / "hello.txt").read_text() == "hello electromind\n"
...
async def test_install_standard_skill_uses_source_and_skill_directory(tmp_path):
    ...
    mount = mounts["my-helper"]
    assert mount.source_root is not None

    # Use the mount's skill_root to verify files — the path includes a digest prefix
    expected_base = mount.skill_root
    assert expected_base.startswith("/home/agent/.skills/")
    assert expected_base.endswith("/my-helper")
...
async def test_install_catalog_rejects_resource_symlink_escape(tmp_path):
    ...
    # The skill with an escaping symlink is rejected at load time
    assert "safe-skill" not in {s.name for s in catalog.registry.list()}
    assert any(d.code == "skill_symlink_rejected" for d in catalog.diagnostics)
    ...
    # No mounts because the skill was rejected
    assert "safe-skill" not in mounts
...
@pytest.mark.asyncio
async def test_changed_catalog_installs_to_new_digest_path(tmp_path):
    """Characterization: Skill 内容变化后重新 install 走到新 digest 路径，旧 digest 被清理。

    安装路径带内容摘要前缀；内容变化 → 新摘要 → 新路径（``new_root != old_root``），
    新挂载持有新内容。当前实现 **每 source 只保留最新 digest**（旧 digest 被
    ``_cleanup_old_digest_dirs`` 剪除）—— 这不是不可变多 digest 存储。
    SKILL-5 单 Skill 懒挂载若要求"只读挂载到 digest 路径、路径稳定"，必须改为
    保留被引用 digest，此测试即锁定现状基线。
    """
    from electromind.skills.discovery import discover_skill_sources, load_skill_catalog
    ...
    sources1 = discover_skill_sources(str(project))
    catalog1 = load_skill_catalog(sources1)

    async with await Sandbox.create(
        backend="local", workdir=str(tmp_path / "box")
    ) as box:
        mounts1 = await box.install_skill_catalog(catalog1)
        old_root = mounts1["my-helper"].skill_root
        assert os.path.isfile(box.resolve(f"{old_root}/SKILL.md"))

        # Change skill content → new source digest
        (d / "SKILL.md").write_text(
            "---\nname: my-helper\ndescription: A helper v2\n---\nChanged body.\n",
            encoding="utf-8",
        )
        catalog2 = load_skill_catalog(discover_skill_sources(str(project)))
        mounts2 = await box.install_skill_catalog(catalog2)
        new_root = mounts2["my-helper"].skill_root

        # Content-addressed: new digest → different path
        assert new_root != old_root
        # Current behavior: old digest dir is pruned after reinstall
        assert not os.path.exists(box.resolve(f"{old_root}/SKILL.md"))
        # New mount carries the new content
        new_md = await box.files.read_text(f"{new_root}/SKILL.md")
        assert "Changed body." in new_md


@pytest.mark.asyncio
async def test_snapshot_verification_rejects_modified_resource(tmp_path):
    """After Snapshot hash is frozen, modifying a resource must fail the mount."""
    from electromind.skills.discovery import discover_skill_sources, load_skill_catalog
    from electromind.skills.snapshot import build_skill_set_snapshot

    project = tmp_path / "project"
    project.mkdir()

    # Standard skill with a nested resource
    agents_skills = project / ".agents" / "skills"
    agents_skills.mkdir(parents=True)
    d = agents_skills / "my-tool"
    d.mkdir()
    refs = d / "references"
    refs.mkdir()
    (d / "SKILL.md").write_text(
    ...
    ref_file = refs / "config.md"
    ref_file.write_text("original config content")

    # Discover and build frozen snapshot
    sources = discover_skill_sources(str(project))
    catalog = load_skill_catalog(sources)
    frozen = build_skill_set_snapshot(catalog, 1)

    # Modify a resource after snapshot
    ref_file.write_text("MODIFIED AFTER SNAPSHOT")

    # Mount must fail because frozen hashes don't match modified content
    async with await Sandbox.create(
        backend="local", workdir=str(tmp_path / "box")
    ) as box:
        with pytest.raises(RuntimeError, match="Staging content mismatch"):
            await box.install_skill_catalog(catalog, skill_set_snapshot=frozen)


@pytest.mark.asyncio
async def test_snapshot_verification_passes_with_frozen_hashes(tmp_path):
    """When files are unchanged, snapshot-based verification passes."""
    from electromind.skills.discovery import discover_skill_sources, load_skill_catalog
    from electromind.skills.snapshot import build_skill_set_snapshot
    ...
    (refs / "config.md").write_text("config content v1")

    sources = discover_skill_sources(str(project))
    catalog = load_skill_catalog(sources)
    frozen = build_skill_set_snapshot(catalog, 1)

    # No modification → mount must succeed
    async with await Sandbox.create(
        backend="local", workdir=str(tmp_path / "box")
    ) as box:
        mounts = await box.install_skill_catalog(catalog, skill_set_snapshot=frozen)
        assert "my-tool" in mounts
