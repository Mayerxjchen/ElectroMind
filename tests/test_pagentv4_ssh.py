...
- 端到端：需要环境变量 `PAGENTV4_SSH_HOST` 配置一个真实可用的 SSH 目标
  才会跑。这是因为 asyncssh 内嵌 server 支持 exec+SFTP 的成本太高，
  真在容器/远端跑更有意义。

环境变量约定（端到端测试）：
    PAGENTV4_SSH_HOST        必填；host
    PAGENTV4_SSH_USER        必填；user
    PAGENTV4_SSH_PORT        选填；默认 22
    PAGENTV4_SSH_PASSWORD    选填
    PAGENTV4_SSH_KEY         选填；私钥路径
    PAGENTV4_SSH_WORKDIR     选填；默认 ~/pagent-test
"""
...
def build_connection() -> dict | None:
    host = os.environ.get("PAGENTV4_SSH_HOST")
    user = os.environ.get("PAGENTV4_SSH_USER")
    if not host or not user:
        return None
    connection: dict = {"host": host, "user": user, "known_hosts": None}
    if port := os.environ.get("PAGENTV4_SSH_PORT"):
        connection["port"] = int(port)
    if password := os.environ.get("PAGENTV4_SSH_PASSWORD"):
        connection["password"] = password
    if key := os.environ.get("PAGENTV4_SSH_KEY"):
        connection["client_keys"] = [key]
    connection["workdir"] = os.environ.get("PAGENTV4_SSH_WORKDIR", "~/pagent-test")
    return connection


needs_ssh = pytest.mark.skipif(
    build_connection() is None,
    reason="set PAGENTV4_SSH_HOST + PAGENTV4_SSH_USER to run",
)
...
async def test_ssh_backend_end_to_end(tmp_path):
    ...
    async with await Sandbox.create(
        ...
        assert box.workdir.endswith("pagent-test") or box.workdir.startswith("/")
